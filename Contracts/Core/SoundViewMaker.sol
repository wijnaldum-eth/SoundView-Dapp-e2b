...
import { ISoundViewMaker } from "./interfaces/ISoundViewMaker.sol";
import { ISoundViewType } from "./interfaces/ISoundViewType.sol";
import { IMetadataModule } from "./interfaces/IMetadataModule.sol";

import { OwnableRoles } from "solady/auth/OwnableRoles.sol";

/**
 * @title SoundViewMaker
 * @notice A factory that deploys minimal proxies of `SoundViewType.sol`.
 * @dev The proxies are OpenZeppelin's Clones implementation of https://eips.ethereum.org/EIPS/eip-1167
 */
contract SoundViewMaker is ISoundViewMaker, OwnableRoles {
    // =============================================================
    //                            STORAGE
    // =============================================================
     ...
     */
    address public soundViewTypeImplementation;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(address _soundViewTypeImplementation) implementationNotZero(_soundViewTypeImplementation) {
        soundViewTypeImplementation = _soundViewTypeImplementation;
    ...
    function createSoundAndMints(
        ...
        bytes[] calldata data
    ) external returns (address soundViewType, bytes[] memory results) {
        // Create Sound Edition proxy.
        soundViewType = payable(Clones.cloneDeterministic(soundViewTypeImplementation, _saltedSalt(msg.sender, salt)));
        ...
        assembly {
            ...
            if iszero(
                call(
                    gas(), // Gas remaining.
                    soundViewType, // Address of the edition.
        ...
        OwnableRoles(soundViewType).transferOwnership(msg.sender);

        emit SoundViewTypeCreated(soundViewType, msg.sender, initData, contracts, data, results);
    }
    ...
    {
        soundViewTypeImplementation = newImplementation;

        emit SoundViewTypeImplementationSet(soundViewTypeImplementation);
    }
